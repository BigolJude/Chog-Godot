...
public partial class GUI : Control
{
    ...
    private struct Challenge
    {
        ...
        {
            internal const string CHARACTER_CONTAINER = CONTAINER + "/Challenge_Options/Character_Container";
        }
    ...
    private static char[] LanguageChars = ['ą', 'č', 'ę', 'ė', 'į', 'š', 'ų', 'ž'];
    private List<Button> Buttons = [.. LanguageChars.Select(x => new Button() { Text = x.ToString() })];
    ...
    [Signal]
    public delegate void OnCharSelectedEventHandler(string text);

    public override void _Ready()
    {
        HSplitContainer CharButtonContainer = GetNode<HSplitContainer>(Challenge.Characters.CHARACTER_CONTAINER);


        foreach (Button button in Buttons)
        {
            CharButtonContainer.AddChild(button);
            button.Pressed += () =>
            {
                LineEdit ChallengeTextBox = GetNode<LineEdit>(Challenge.Container.LINE_EDIT);
                GD.Print(ChallengeTextBox.CaretColumn);
                CaretPosition = ChallengeTextBox.CaretColumn;
                ChallengeTextBox.Text = ChallengeTextBox.Text[..ChallengeTextBox.CaretColumn] + button.Text + ChallengeTextBox.Text[ChallengeTextBox.CaretColumn..];
                ChallengeTextBox.CaretColumn = CaretPosition + 1;
                OnChallengeTextChanged(string.Empty);
            };
        }
    ...
    public override void _Process(double delta)
    {
        ...
        if (Input.IsActionJustPressed("capslock"))
        {
            IsCapslockOn = !IsCapslockOn;

            foreach (Button child in CharButtonContainer.GetChildren())
            {
                child.Text = IsCapslockOn
                  ? child.Text.ToUpper()
                  : child.Text.ToLower();
            }
        }
    ...
    private void OnChallengeTextChanged(string _)
    {
        LineEdit ChallengeTextBox = GetNode<LineEdit>(Challenge.Container.LINE_EDIT);
    ...
    private void OnCharSelect(string text)
    {
        GD.Print(text);
    }
}
